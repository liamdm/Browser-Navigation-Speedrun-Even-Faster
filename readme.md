...
Note: This will remain a valid solution imo till the site actually attempts to implement some form of true anti-scrape. The level of anti-scrape on this site is significantly worse than the majority of existing anti-scrape tools tbh - and most of the time for a browser navigation challenge the real key is "trying to bypass the antiscrape" rather than trying to get the LLM to figure out where to click - naturally as exceptional language task executors, LLMs have a great ability to "finding the right element on a page". I guess that's why there's the 5s time limit to "spice things up"?
